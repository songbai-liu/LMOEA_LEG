        ...
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(Population) 
                ...
                for i = 1 : Problem.N
                    ...
                    if rand > t  %search in the compressed representation space
                        child = (child-Lower)./(Upper-Lower);
                        Parent1 = (Parent1-Lower)./(Upper-Lower);
                        Parent2 = (Parent2-Lower)./(Upper-Lower);
                        codec = mlp.encode(child);
                        code1 = mlp.encode(Parent1);
                        code2 = mlp.encode(Parent2);
                        code = codec{end} + 0.5*(code1{end}-code2{end});
						aa = find(code<0);
                        bb = find(code>1);
                        code(aa) = rand*0.001;
                        code(bb) = rand*0.001 + 0.999;
                        GDV = mlp.decode(code);
